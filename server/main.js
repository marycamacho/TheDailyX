(function () {
    ...
    require('./server/mongo-connection')(function (err, dbConnection) {
        ...
        app.post('/signup', function (req, res, next) {
            if (!req.body.type) {
                return next({'clientMessage': 'errorParameterNotSpecified', 'messageData': {'#1#': 'type'}});
            }
            if (req.body.type == 'facebook') {
                require('./server/api/users/authenticateFacebook')('signup')(req, res, next);
            } else {
             }
        });
        ...
        app.post('/signin', function (req, res, next) {
            if (req.body.type == 'facebook') {
                require('./server/api/users/authenticateFacebook')('signin')(req, res, next);
            } else {
                require('./server/api/users/authenticatePassword')('signin')(req, res, next);
            }
        });
